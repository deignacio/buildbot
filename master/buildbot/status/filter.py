...
class StatusFilter(ComparableMixin):
    ...
    def __init__(self,
                 ...
                 result=None, result_re=None, result_fn=None):
        """
        ...
        @type  result: string or list
        @param result: result(s) to filter on
        
        @type  result_re: regular expression
        @param result_re: matches result

        @type  result_fn: function
        @param result_fn: takes the result, returns a boolean
        """
        def mklist(x):
        ...
        self.result_checks = (mklist(result), mkre(result_re), result_fn)
        self.checks = [
    ...
    def filter_status(self, builderStatus, result):
        if self.filter_fn is not None and not self.filter_fn(builderStatus):
            return False
        if not self._filter_value(result, *self.result_checks):
            return False
